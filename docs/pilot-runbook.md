...
1. Deploy the migration set, including `20260811160000_stadium_pilot_controls`.
...
3. Move to `closed` only after the director approves operational closeout. Archive only after review.
4. Preserve the EventAuditLog as the official record of event creation, lifecycle changes, readiness changes, and issue activity. It is database-immutable.

## 5. Pilot limitations and safety controls

- Offline mutation queue, idempotent sync, canonical event transfers/86s/counts, and persistent closeout financials are not yet enabled. Keep a paper or approved manual fallback if venue connectivity is unreliable.
